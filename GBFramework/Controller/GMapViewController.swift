...
class GMapViewController: UIViewController {
    ...
    private func savePath(path: GMSMutablePath?) {
    ...
    private func loadPath() -> GMSMutablePath? {
        ...
        do {
            ...
            if let encodedPath = objects.first?.encodedPath {
                return GMSMutablePath(fromEncodedPath: encodedPath)
            }
        } catch _ {
            print("Error deleting paths!")
        }
    ...
    private func showLastRoute() -> Bool {
        ...
        guard let path = loadPath()
        else { return false }
        
        mapView.clear()
        configureRoute(path: path)
        guard let count = routePath?.count()
        else { return false }
        if count > 0 {
